    ...
    <div className="p-6 sm:p-8 rounded-2xl bg-background border border-hairline shadow-xs space-y-6">
      <div className="flex items-center justify-between border-b border-hairline pb-4">
        ...
        <button
          ...
          className="px-4 py-2 rounded-full bg-foreground hover:bg-foreground/90 text-background font-bold text-xs transition-colors flex items-center gap-1.5 shadow-xs cursor-pointer"
        >
      ...
      {loading ? (
        <div className="p-6 text-center text-xs text-muted-foreground">Loading saved destinations...</div>
      ) : addresses.length === 0 ? (
        <div className="p-6 text-center rounded-xl bg-surface/50 border border-hairline space-y-2">
          <MapPin className="w-6 h-6 text-muted-foreground mx-auto" />
          <h4 className="text-xs font-bold text-foreground">No Saved Addresses</h4>
          <p className="text-[11px] text-muted-foreground">Add a US shipping destination for faster checkout.</p>
          <button
            type="button"
            onClick={handleOpenAddModal}
            className="px-3.5 py-1.5 rounded-full bg-foreground text-background text-xs font-bold shadow-xs cursor-pointer"
          >
            Add New Address
          </button>
        </div>
          ...
          >
            ...
            <form onSubmit={handleSaveAddress} className="space-y-4">
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  type="checkbox"
                  id="isDefaultAddrCheckSection"
                  checked={isDefault}
                  onChange={(e) => setIsDefault(e.target.checked)}
                  className="rounded border-hairline text-foreground focus:ring-foreground cursor-pointer"
                />
                <label htmlFor="isDefaultAddrCheckSection" className="text-xs text-muted-foreground cursor-pointer select-none font-semibold">
